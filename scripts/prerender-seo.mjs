...
function bodyFor(path){
  ...
  if(city){const [slug,name,note]=city;return `<main data-jeriko-prerender="true"><section><p>Full-service bilingual remodeling near ${name}.</p><h1>${name} remodeling services for homeowners</h1><p>Brothers Remodeling OKC helps homeowners around ${name} with kitchen remodeling, bathroom remodeling, flooring, drywall, painting, exterior repairs, doors, windows, garages, and whole-home updates.</p><p>Common requests include ${note}. English and Spanish communication is available.</p><p><a href="/contact">Request a ${name} remodeling quote</a> or <a href="mailto:brothersremodelingokc@gmail.com" data-jeriko-track="email_click">email project photos</a>.</p><form action="/api/contact" method="post" data-jeriko-track="form_submit" aria-label="${name} remodel quote request"><input name="name" autocomplete="name" required /><input name="phone" type="tel" autocomplete="tel" required /><input name="city" value="${name}" /><textarea name="message" required></textarea><button type="submit">Request quote</button></form></section></main>`}
